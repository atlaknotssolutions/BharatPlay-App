...
export default function ShortsScreen() {
  ...
  const loadShorts = useCallback(async () => {
    try {
      ...
      const response = await fetch(`${API_USERVIDEO}/trending-shorts`, {
        headers: token
          ? {
  ...
  const handleSubscribe = async (item: ShortItem) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        Alert.alert("Success", data.message || "Subscribed successfully");
      } else {
  ...
  const renderItem: ListRenderItem<ShortItem> = useCallback(
    ...
    [currentIndex, liked, likeCount, muted, trackView],
  );
